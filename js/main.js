...
function startFunc() {
   ...
   if(!window.jQuery) {
    ...
    script.addEventListener('load', ()=> { scriptLoad2(); });
...
// Загрузка внешнего скрипта
  function scriptLoad2() {
     const script = document.createElement('script');
    script.type = "text/javascript";
    script.src = 'libs/slick/slick.min.js';
    
    script.addEventListener('load', ()=> {  scriptLoadValidHeader(); });
    document.body.append(script);
  }
function   scriptLoadValidHeader()
{
  //запуск слик слайдер
$('.container-section-payments-slider').slick();
   const script = document.createElement('script');
    script.type = "text/javascript";
    script.src = 'js/validateheader.js';
    
    script.addEventListener('load', ()=> {  scriptLoadValidBody();  });
    document.body.append(script);
}
function   scriptLoadValidBody()
{
   const script = document.createElement('script');
    script.type = "text/javascript";
